...
int SendMetrics(DbmsCtx* ctx)
{
    ...
    SendMetric(ctx, 35, ctx->model.max_t);
    SendMetric(ctx, 36, ctx->model.min_t);
    SendMetric(ctx, 37, ctx->model.avg_t);
    SendMetric(ctx, 38, ctx->model.max_v);
    SendMetric(ctx, 39, ctx->model.min_v);
    SendMetric(ctx, 40, ctx->model.avg_v);
    // TODO: perm sol.
