...
void BridgeSetFaultSummary(DbmsCtx* ctx, uint8_t addr, uint8_t fault_summary_reg);
...
// void StackSetFault(DbmsCtx* ctx, uint8_t addr, MonitorFaultType fault); // todo: make it so each category has its own fault mask
void StackClearFault(DbmsCtx* ctx, uint8_t addr, MonitorFaultType fault);
bool StackHasFault(DbmsCtx* ctx, uint8_t addr, MonitorFaultType fault);
