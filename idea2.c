...
typedef struct PACKED _TxFrame 
{
...
typedef struct PACKED _RxFrame
{
...
/**
 * Grabs CRC from real location is at f->data + f->len
 */
#define GET_CRC(FRAME)  ((FRAME->data + FRAME->len)[0] << 8) + ((FRAME->data + FRAME->len)[1])

/**
 * Helpful macros to make frames and such 
 * 
 */
...
// becomes:
static TxFrame frame2 = MAKE_TX_FRAME(REQ_STACK_WRITE, 0, 0x0E09, 0x2D, 0x09 );
